              ...
              <Link key={cat.id} href={`/urunler/${cat.slug}`}
                ...
                {cat.description && (
                  <div className="px-4 py-3">
                    <p className="line-clamp-2 text-xs text-zinc-500">{cat.description}</p>
                  </div>
                )}
              </Link>
            ))}
            ...
            {[
              {
                ...
                desc: "Tüm ürünlerimiz uluslararası standartlara uygun olarak tasarlanmış ve hassas ölçüm sonuçları için optimize edilmiştir.",
              },
              {
                icon: "🏆",
                title: "Sektör Deneyimi",
                desc: "20 yılı aşkın deneyimimizle endüstriyel test cihazları alanında güvenilir çözümler sunuyoruz.",
              },
              {
                icon: "🔧",
                title: "Teknik Destek",
                desc: "Satış sonrası teknik destek ve bakım hizmetlerimizle yanınızdayız. Uzman ekibimiz her zaman erişilebilir.",
              },
              {
                icon: "📋",
                title: "Uluslararası Standartlar",
                desc: "TS, EN, ASTM, BS ve ISO standartlarına tam uyumluluk. Her ürün sertifikalı ve onaylı.",
              },
              {
                icon: "⚡",
                title: "Hızlı Teslimat",
                desc: "Stok yönetimimiz ve lojistik ağımız sayesinde en kısa sürede teslimat sağlıyoruz.",
              },
              {
                icon: "💡",
                title: "Ar-Ge Odaklı",
                desc: "Sürekli Ar-Ge yatırımlarımızla teknolojinin ön saflarında yer alıyor, yenilikçi çözümler geliştiriyoruz.",
              },
            ].map((item) => (
              <div key={item.title}
      ...
      <section className="bg-white px-4 py-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <div className="mt-8 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            <Link href="/iletisim"
              className="inline-flex items-center gap-2 rounded-full bg-navy-950 px-8 py-4 font-bold text-white transition-all hover:bg-navy-800 hover:shadow-lg hover:shadow-navy-950/20">
              İletişime Geçin
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <path d="M5 12h14M12 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
