...
export default function Header() {
  ...
  const navLinkClass = (active: boolean) =>
    `relative flex items-center gap-1 px-4 py-2 text-sm font-semibold transition-colors rounded-lg
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center md:flex">
            ...
            <div className="relative"
              ...
              <button className={navLinkClass(isKurumActive)}>
                Kurumsal
                <svg width="11" height="11" viewBox="0 0 12 12" fill="currentColor"
                  className={`transition-transform duration-200 ${kurumOpen ? "rotate-180" : ""}`}>
                  <path d="M6 8L1 3h10z"/>
                </svg>
                {isKurumActive && <span className="absolute bottom-0 left-4 right-4 h-0.5 rounded-full bg-accent" />}
              </button>
              {kurumOpen && (
                <div className="absolute left-0 top-full w-52 pt-2">
                  <div className="overflow-hidden rounded-xl border border-white/10 bg-navy-900 shadow-2xl shadow-black/40">
                    <div className="p-1">
                      {[
                        { href: "/kurumsal/hakkimizda", icon: "🏢", label: "Hakkımızda" },
                        { href: "/kurumsal/vizyon-misyon", icon: "🎯", label: "Vizyon & Misyon" },
                      ].map(({ href, icon, label }) => (
            ...
            <div className="relative"
              ...
              style={{ paddingBottom: "8px" }}>
              <Link href="/urunler" className={navLinkClass(isProductsActive)}>
                Ürünler
                <svg width="11" height="11" viewBox="0 0 12 12" fill="currentColor"
                  className={`transition-transform duration-200 ${productsOpen ? "rotate-180" : ""}`}>
                  <path d="M6 8L1 3h10z"/>
                </svg>
                {isProductsActive && <span className="absolute bottom-0 left-4 right-4 h-0.5 rounded-full bg-accent" />}
              </Link>
              {productsOpen && (
            ...
            <Link href="/hizmetlerimiz" className={navLinkClass(isActive("/hizmetlerimiz"))}>
              Hizmetlerimiz
              {isActive("/hizmetlerimiz") && <span className="absolute bottom-0 left-4 right-4 h-0.5 rounded-full bg-accent" />}
            </Link>

            <Link href="/cozumlerimiz" className={navLinkClass(isActive("/cozumlerimiz"))}>
              Çözümlerimiz
...
<Link href="/standartlar" className={navLinkClass(isActive("/standartlar"))}>
  ...
  {isActive("/standartlar") && <span className="absolute bottom-0 left-4 right-4 h-0.5 rounded-full bg-accent" />}
        ...
        {/* Navbar altı ince gradient çizgi */}
