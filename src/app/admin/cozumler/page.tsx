...
export default function AdminCozumlerPage() {
  ...
  async function load() {
    const { data } = await createClient().from("solutions").select("*").order("order");
    setSolutions(data || []);
  }
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      order: editing.order ?? solutions.length,
      is_active: editing.is_active ?? true,
    };
    if (editing.id) {
      await sb.from("solutions").update(payload).eq("id", editing.id);
    } else {
      await sb.from("solutions").insert(payload);
    }
    setSaving(false);
    setEditing(null);
    load();
  ...
  async function handleDelete(id: string) {
    ...
    await createClient().from("solutions").delete().eq("id", id);
    load();
  }

  async function toggleActive(s: Solution) {
    await createClient().from("solutions").update({ is_active: !s.is_active }).eq("id", s.id);
    load();
  }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-navy-950">Çözümlerimiz</h1>
          <p className="text-sm text-zinc-500 mt-1">Çözümler sayfasındaki içerikleri yönetin.</p>
        </div>
        <button
          onClick={() => setEditing({ title: "", subtitle: "", description: "", media_type: "youtube", youtube_url: "", order: solutions.length, is_active: true })}
      ...
      {editing && (
        <div className="mb-6 rounded-xl bg-white border border-zinc-200 p-6">
          <h2 className="mb-4 font-bold text-navy-950">{editing.id ? "Çözüm Düzenle" : "Yeni Çözüm"}</h2>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <input value={editing.title || ""} onChange={e => setEditing(v => ({ ...v!, title: e.target.value }))}
                  placeholder="HP-Q1000 Kutu Karekodlama Makinası"
              ...
              <div>
                ...
                <input value={editing.subtitle || ""} onChange={e => setEditing(v => ({ ...v!, subtitle: e.target.value }))}
                  placeholder="Endüstriyel karekodlama çözümü"
                  className="w-full rounded-lg border border-zinc-300 px-3 py-2.5 text-sm focus:border-navy-700 focus:outline-none" />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea value={editing.description || ""} onChange={e => setEditing(v => ({ ...v!, description: e.target.value }))} rows={4}
                placeholder="Çözüm hakkında detaylı açıklama..."
                className="w-full rounded-lg border border-zinc-300 px-3 py-2.5 text-sm focus:border-navy-700 focus:outline-none" />
            </div>
                ...
                {[
                  ...
                  <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="mediaType" value={opt.value}
                      ...
                      onChange={() => setEditing(v => ({ ...v!, media_type: opt.value as Solution["media_type"] }))}
              ...
              <div>
                ...
                {editing.media_url && (
                  <div className="relative mb-2 h-40 w-full overflow-hidden rounded-lg border border-zinc-200">
                    <Image src={editing.media_url} alt="Çözüm görseli" fill className="object-cover" sizes="600px" />
                ...
                {editing.youtube_url && getYoutubeId(editing.youtube_url) && (
                  <div className="mt-2 aspect-video w-full max-w-sm overflow-hidden rounded-lg bg-zinc-900">
                    <iframe width="100%" height="100%"
                      ...
                      title="Önizleme" allowFullScreen />
                  </div>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-center gap-2 pt-5">
                <input type="checkbox" id="solActive" checked={editing.is_active ?? true}
                  onChange={e => setEditing(v => ({ ...v!, is_active: e.target.checked }))}
                  className="h-4 w-4 rounded border-zinc-300" />
                <label htmlFor="solActive" className="text-sm font-medium text-zinc-700">Aktif (sitede görünsün)</label>
              </div>
            </div>
          </div>
            ...
            <div key={s.id} className="flex items-center gap-4 rounded-xl bg-white border border-zinc-200 p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-zinc-400 mt-0.5">
                  Sıra: {s.order} · {s.media_type === "youtube" ? "YouTube" : s.media_type === "image" ? "Görsel" : "Medya yok"}
                </p>
              </div>
